...
var Hypermesh = function(N, svg, verticies, edges, scale) {
	...
	this.labelSVG = svg.selectAll("text")
	    .data(this.verticies)
	    .enter()
	    .append("text")
	    .text(function (d){
...
Hypermesh.prototype.remove = function(){
    ...
    this.labelSVG.map(function(u){u.remove();});
}
...
Hypermesh.prototype.render = function(){
	...
	_.zip(that.verticies, that.labelSVG).map(function(vertex){
			var vSVG = vertex[1];
			var v = vertex[0];
			vSVG.setAttribute("x", that.getX(v));
			vSVG.setAttribute("y", that.getY(v));
		});
